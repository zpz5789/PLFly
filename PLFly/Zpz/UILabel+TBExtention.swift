...
public extension UILabel {
    ...
    ///   - fontWeight: 字体权重
    /// - Returns: 返回自身
    func font(_ fontSize: CGFloat, _ fontWeight: UIFont.Weight) -> Self {
...
// MARK: - 二、其他的基本扩展
public extension TBPOP where Base: UILabel {
    ...
    func accordWidthLinesCountAndLinesContent(accordWidth: CGFloat, lineSpace: CGFloat, textSpace: CGFloat = 0.0, paraSpace: CGFloat = 0.0) -> (Int?, [String]?) {
        ...
        let contentDict = UILabel.tb.genTextStyle(text: t as NSString, linebreakmode: NSLineBreakMode.byCharWrapping, align: align, font: f, lineSpace: lineSpace, textSpace: textSpace, paraSpace: paraSpace)
...
// MARK: - 四、计算宽高，自适应尺寸
public extension TBPOP where Base: UILabel {
    // MARK: 4.1、根据label高度计算宽度，高度固定
    /// 根据label高度计算宽度，高度固定
    /// - Returns: 宽度
    func calculateWidth() -> CGFloat {
        base.lineBreakMode = .byCharWrapping
        let size = calculateLabelSizeWithTitle(base.text ?? "", font: base.font, constrainedToSize: CGSize(width: CGFloat(MAXFLOAT), height: base.frame.size.height), lineBreakMode: .byCharWrapping)
        return ceil(size.width)
    }
    
    // MARK: 4.2、根据label高度计算高度，宽度固定
    /// 根据label高度计算宽度，高度固定
    /// - Returns: 高度
    func calculateHeightWithWidth(_ width: CGFloat) -> CGFloat {
        base.numberOfLines(0).lineBreakMode(.byCharWrapping)
        let size = calculateLabelSizeWithTitle(base.text ?? "", font: base.font, constrainedToSize: CGSize(width: width, height: CGFloat(MAXFLOAT)), lineBreakMode: .byWordWrapping)
        return ceil(size.height)
    }
    
    // MARK: 4.3、根据指定宽度和行高计算高度
    /// 根据指定宽度和行高计算高度
    /// - Parameters:
    ///   - width: 宽度
    ///   - lineHeight: 行高
    /// - Returns: 高度
    func calculateHeightWithWidth(_ width: CGFloat,oneLineHeight height: CGFloat) -> CGFloat {
        let newHeight = calculateHeightWithWidth(width)
        return newHeight * height / base.font.lineHeight
    }
    
    // MARK: 4.4、自适应宽度
    /// 自适应宽度
    /// - Returns: 宽度
    func autoAdaptWidth() -> CGFloat {
        let size = base.sizeThatFits(CGSize(width: CGFloat(MAXFLOAT), height: base.bounds.size.height))
        return ceil(size.width)
    }
    ...
    /// 自适应高度
    /// - Returns: 高度
    func autoAdaptHeight() -> CGFloat {
        let size = base.sizeThatFits(CGSize(width:base.bounds.size.width, height:  CGFloat(MAXFLOAT)))
        return size.height
    }
    ...
    /// 自适应大小
    /// - Returns: 文本大小
    func autoAdaptSize() -> CGSize {
        let size = base.sizeThatFits(CGSize(width:CGFloat(MAXFLOAT), height:  CGFloat(MAXFLOAT)))
        return size
    }
    
    /// 获取文字尺寸
    private func calculateLabelSizeWithTitle(_ title: String, font:UIFont, constrainedToSize size:CGSize, lineBreakMode: NSLineBreakMode) -> CGSize {
        if title.count == 0 {
            return CGSize.zero
        }
        let paragraph = NSMutableParagraphStyle()
        paragraph.lineBreakMode = lineBreakMode
        let frame = NSString(string: title).boundingRect(with: size, options: NSStringDrawingOptions.usesLineFragmentOrigin, attributes: [NSAttributedString.Key.font: font, NSAttributedString.Key.paragraphStyle: paragraph],context: nil)
        return frame.size
    }
}
