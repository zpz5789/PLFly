...
extension UIImage: TBPOPCompatible {}
// MARK: - 一、基本的扩展
public extension TBPOP where Base: UIImage {
    ...
    static func textImage(_ text: String, fontSize: CGFloat = 16, size: (CGFloat, CGFloat), backgroundColor: UIColor = UIColor.orange, textColor: UIColor = UIColor.white, isCircle: Bool = true, isFirstChar: Bool = false) -> UIImage? {
        ...
        let textsize = text.tb.rectSize(font: UIFont.systemFont(ofSize: fontSize), size: CGSize(width: UIScreen.main.bounds.width, height: CGFloat(MAXFLOAT)))
...
fileprivate extension UIImage {
    ...
    private struct TBRuntimeKey {
        static let saveBlockKey = UnsafeRawPointer(bitPattern: "saveBlock".hashValue)
    }
    private var saveBlock: ((Bool)->())? {
        set {
            objc_setAssociatedObject(self, TBRuntimeKey.saveBlockKey!, newValue, .OBJC_ASSOCIATION_COPY_NONATOMIC)
        }
        get {
            return objc_getAssociatedObject(self, TBRuntimeKey.saveBlockKey!) as? (Bool) -> ()
        }
    }
...
public enum TBImageGradientDirection {
    /// 水平从左到右
    case horizontal
    /// 垂直从上到下
    case vertical
...
public extension TBPOP where Base: UIImage {
    ...
    ///   - direction: 渐变的方向
    /// - Returns: 渐变的图片
    static func gradient(_ hexsString: [String], size: CGSize = CGSize(width: 1, height: 1), locations:[CGFloat]? = nil, direction: TBImageGradientDirection = .horizontal) -> UIImage? {
    ...
    ///   - direction: 渐变的方向
    /// - Returns: 渐变的图片
    static func gradient(_ colors: [UIColor], size: CGSize = CGSize(width: 10, height: 10), locations:[CGFloat]? = nil, direction: TBImageGradientDirection = .horizontal) -> UIImage? {
    ...
    static func gradient(_ colors: [UIColor],
                         ...
                         direction: TBImageGradientDirection = .horizontal) -> UIImage? {
    ...
    func solidTo(size: CGSize) -> UIImage? {
        ...
        if self.base.size.height <= self.base.size.width {
            if self.base.size.width >= w {
                let scaleImage = fixOrientation().tb.scaleTo(size: CGSize(width: w, height: w * self.base.size.height / self.base.size.width))
                return scaleImage
            } else {
            ...
            if self.base.size.height >= h {
                let scaleImage = fixOrientation().tb.scaleTo(size: CGSize(width: h * self.base.size.width / self.base.size.height, height: h))
                return scaleImage
            } else {
                return fixOrientation()
            }
...
public extension TBPOP where Base: UIImage {
    ...
    func compress(mode: CompressionMode = .medium) -> Data? {
        return resizeIO(resizeSize: mode.resize(base.size))?.tb.compressDataSize(maxSize: mode.maxDataSize)
    }
    ...
    func asyncCompress(mode: CompressionMode = .medium, queue: DispatchQueue = DispatchQueue.global(), complete:@escaping (Data?, CGSize) -> Void) {
        queue.async {
            let data = resizeIO(resizeSize: mode.resize(self.base.size))?.tb.compressDataSize(maxSize: mode.maxDataSize)
            DispatchQueue.main.async {
...
public extension TBPOP where Base: UIImage {
    ...
    private static func getHDImgWithCIImage(with image: CIImage, size: CGSize, isLogo: Bool = true, logoSize: CGSize?, logoImage: UIImage? = nil, logoRoundCorner: CGFloat? = nil) -> UIImage? {
        ...
        if let newLogoRoundCorner = logoRoundCorner, let roundCornerLogo = logoImage.tb.isRoundCorner(radius: newLogoRoundCorner, byRoundingCorners: .allCorners, imageSize: logoSize) {
...
public extension TBPOP where Base: UIImage {
    ...
    static func gif(data: Data) -> UIImage? {
        ...
        guard let source = CGImageSourceCreateWithData(data as CFData, nil) else {
            TBPrint("SwiftGif: Source for the image does not exist")
            return nil
        }
    ...
    static func gif(url: String) -> UIImage? {
        ...
        guard let bundleURL = URL(string: url) else {
            TBPrint("SwiftGif: This image named \"\(url)\" does not exist")
            return nil
        }
        // Validate data
        guard let imageData = try? Data(contentsOf: bundleURL) else {
            TBPrint("SwiftGif: Cannot turn image named \"\(url)\" into NSData")
            return nil
        }
    ...
    static func gif(name: String) -> UIImage? {
        ...
        guard let bundleURL = Bundle.main
                .url(forResource: name, withExtension: "gif") else {
                    TBPrint("SwiftGif: This image named \"\(name)\" does not exist")
                    return nil
                }
        // Validate data
        guard let imageData = try? Data(contentsOf: bundleURL) else {
            TBPrint("SwiftGif: Cannot turn image named \"\(name)\" into NSData")
            return nil
        }
    ...
    static func gif(asset: String) -> UIImage? {
        ...
        guard let dataAsset = NSDataAsset(name: asset) else {
            TBPrint("SwiftGif: Cannot turn image named \"\(asset)\" into NSDataAsset")
            return nil
        }
...
public extension TBPOP where Base: UIImage {
    ...
    ///   - alpha: 透明度
    /// - Returns: 添加滤镜后的图片
    func filter(filterType: TBImageFilterType, alpha: CGFloat?) -> UIImage? {
