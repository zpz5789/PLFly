...
extension Date: TBPOPCompatible {}
/// 时间戳的类型
public enum TBTimestampType: Int {
    /// 秒
    case second
    /// 毫秒
    case millisecond
...
// MARK: - 一、Date 基本的扩展
public extension TBPOP where Base == Date {
    ...
    /// 从日期获取 星期
    var weekday: String {
        tb_formatter.dateFormat = "EEEE"
        return tb_formatter.string(from: self.base)
    }
...
public extension TBPOP where Base == Date {
    ...
    static func timestampToFormatterTimeString(timestamp: String, format: String = "yyyy-MM-dd HH:mm:ss") -> String {
        ...
        // 设置 dateFormat
        tb_formatter.dateFormat = format
        // 按照dateFormat把Date转化为String
        return tb_formatter.string(from: date)
    }
    ...
    static func timestampToFormatterDate(timestamp: String) -> Date {
        ...
        guard let timestampInt = timestamp.tb.toInt() else {
            #if DEBUG
            fatalError("时间戳位有问题")
            #else
            return Date()
    ...
    func toformatterTimeString(formatter: String = "yyyy-MM-dd HH:mm:ss") -> String {
        // let dateFormatter = DateFormatter()
        tb_formatter.timeZone = TimeZone.autoupdatingCurrent
        tb_formatter.dateFormat = formatter
        return tb_formatter.string(from: self.base)
    }
    ...
    /// - Returns: 返回转化后的时间戳
    static func formatterTimeStringToTimestamp(timesString: String, formatter: String, timestampType: TBTimestampType = .second) -> String {
        tb_formatter.dateFormat = formatter
        guard let date = tb_formatter.date(from: timesString) else {
            #if DEBUG
            fatalError("时间有问题")
            #else
            return ""
    ...
    ///   - type: 格式类型
    /// - Returns: 返回时间条
    static func getFormatPlayTime(seconds: Int, type: TBTimeBarType = .normal) -> String {
        if seconds <= 0{
            return "00:00"
        }
        // 秒
    ...
    /// - Returns: 时间戳
    func dateToTimeStamp(timestampType: TBTimestampType = .second) -> String {
        // 10位数时间戳 和 13位数时间戳
...
public extension TBPOP where Base == Date {
    ...
    var isYesterday: Bool {
        // 1.先拿到昨天的 date
        guard let date = Base.tb.yesterDayDate else {
            return false
        }
        // 2.比较当前的日期和昨天的日期
        return Calendar.current.isDate(self.base, inSameDayAs: date)
    ...
    var isTheDayBeforeYesterday: Bool  {
        // 1.先拿到前天的 date
        guard let date = Base.tb.theDayBeforYesterDayDate else {
            return false
        }
        // 2.比较当前的日期和昨天的日期
        return Calendar.current.isDate(self.base, inSameDayAs: date)
...
public extension TBPOP where Base == Date {
    ...
    static func currentMonthDays() -> Int {
        return daysCount(year: Date.tb.currentDate.tb.year, month: Date.tb.currentDate.tb.month)
    }
}
