...
extension String: TBPOPCompatible {}
extension NSString: TBPOPCompatible {}

// MARK: - 一：字符串基本的扩展
public extension TBPOP where Base: ExpressibleByStringLiteral {
    ...
    func toClass() -> AnyClass? {
        ...
        guard let Class: AnyClass = NSClassFromString(namespace.tb.removeSomeStringUseSomeString(removeString: " ", replacingString: "_") + "." + (base as! String)) else {
            return nil
        }
        return Class
    }
...
public extension TBPOP where Base: ExpressibleByStringLiteral {
    ...
    var toNumber: NSNumber? {
        return self.toDouble()?.tb.number
    }
}

// MARK: - 五、字符串UI的处理
extension TBPOP where Base: ExpressibleByStringLiteral {
...
public extension TBPOP where Base: ExpressibleByStringLiteral {
    ...
    func accuraterDouble() -> Double? {
        ...
        TBPrint(NSDecimalNumber(decimal: decimal).doubleValue)
...
extension TBPOP where Base: ExpressibleByStringLiteral {
    ...
    public var isValidIDCardNumStrict: Bool {
        ...
        case 15:
            ...
            year = Int(str.tb.sub(start: 6, length: 2))!
        ...
        case 18:
            // 18位身份证
            year = Int(str.tb.sub(start: 6, length: 4))!
            ...
            if numberOfMatch > 0 {
                ...
                for i in 0 ..< 17 {
                    if let d = Int(str.tb.slice(i ..< (i + 1))) {
                        s += d * jiaoYan[i % 10]
                    } else {
                        return false
                    }
                ...
                let Y = s % 11
                let JYM = "10X98765432"
                let M = JYM.tb.sub(start: Y, length: 1)
                if M == str.tb.sub(start: 17, length: 1) {
                    return true
                } else {
                    return false
                }
    ...
    public func hideEmail(combine: String = "*", digitsBefore: Int = 1, digitsAfter: Int = 1) -> String {
        let emailArray = (base as! String).tb.separatedByString(with: "@")
        if emailArray.count == 2 {
            let fistContent = emailArray[0]
            let encryptionContent = fistContent.tb.hidePhone(combine: "*", digitsBefore: 1, digitsAfter: 1)
    ...
    public func stringWithHighLightSubstring(keyword: String, font: UIFont, normalColor: UIColor, keywordCololor: UIColor, isSplit: Bool = false, options: NSRegularExpression.Options = []) -> NSMutableAttributedString {
        ...
        if isSplit {
            for i in 0..<keyword.count {
                let singleString = keyword.tb.sub(start: i, length: 1)
                let ranges = TBRegexHelper.matchRange(totalString, pattern: singleString)
                for range in ranges {
                    attributedString.addAttributes([.foregroundColor: keywordCololor, .font: font], range: range)
                }
            }
        } else {
            let ranges = TBRegexHelper.matchRange(totalString, pattern: keyword)
            for range in ranges {
                attributedString.addAttributes([.foregroundColor: keywordCololor, .font: font], range: range)
            }
        }
...
extension TBPOP where Base: ExpressibleByStringLiteral {
    ...
    public func nsRange(of subString: String) -> [NSRange] {
        guard (base as! String).tb.contains(find: subString) else {
            return []
        }
        // return text.range(of: subString)
        return TBRegexHelper.matchRange(base as! String, pattern: subString)
    }
    ...
    public func insertString(content: String, locat: Int) -> String {
        ...
        let str1 = (base as! String).tb.sub(to: locat)
        let str2 = (base as! String).tb.sub(from: locat)
    ...
    public func matchesMiddleContentOfCharacters(leftChar: String, rightChar: String) -> [String] {
        do {
            ...
            let nsString = (base as! String).tb.toNSString
            let results = regex.matches(in: base as! String, range: NSRange(location: 0, length: nsString.length))
            return results.map {
                nsString.substring(with: $0.range)
                // (base as! String).tb.sub(start: $0.range.location, length: $0.range.length)
...
public extension TBPOP where Base: ExpressibleByStringLiteral {
    ...
    func binaryToOctal() -> String {
        ...
        // 八进制
        return decimal.tb.decimalToOctal()
    }
    ...
    func binaryToHexadecimal() -> String {
        ...
        let decimal = binary.binaryTodecimal()
        // 十六进制
        return decimal.tb.decimalToHexadecimal()
    }
    ...
    func octalTobinary() -> String {
        ...
        // 二进制
        return decimal.tb.decimalToBinary()
    }
    ...
    func octalToHexadecimal() -> String {
        ...
        // 十进制
        let decimal = octal.tb.octalTodecimal()
        // 十六进制
        return decimal.tb.decimalToHexadecimal()
    }
    ...
    func decimalToBinary() -> String {
        guard var decimal = (self.base as! String).tb.toInt() else {
            return ""
        }
        var str = ""
        while decimal > 0 {
    ...
    func decimalToOctal() -> String {
        guard let decimal = (self.base as! String).tb.toInt() else {
            return ""
        }
        /*
...
public extension TBPOP where Base: ExpressibleByStringLiteral {
    ...
    func md5Encrypt(_ md5Type: MD5EncryptType = .lowercase32) -> String {
        guard (self.base as! String).count > 0 else {
            TBPrint("⚠️⚠️⚠️md5加密无效的字符串⚠️⚠️⚠️")
            return ""
        }
...
public extension TBPOP where Base == String {
