...
extension UIDevice: TBPOPCompatible {}
// 这里只指屏幕类型
...
// MARK: - 一、基本的扩展
public extension TBPOP where Base: UIDevice {
    ...
    static func screenType() -> UIDeviceScreenType {
        let modelName = Base.tb.modelName
    ...
    static func isIphone5Screen() -> Bool {
        if Base.tb.screenType() == .IPHONE_5 {
            return true
        }
        return false
    }
    ...
    static func isIphone6Screen() -> Bool {
        if Base.tb.screenType() == .IPHONE_6 {
            return true
        }
        return false
    }
    ...
    static func isIphoneXScreen() -> Bool {
        if Base.tb.screenType() == .IPHONE_X || Base.tb.screenType() == .IPHONE_XS || Base.tb.screenType() == .IPHONE_XR || Base.tb.screenType() == .IPHONE_XS_Max ||  Base.tb.screenType() == .IPHONE_11 ||  Base.tb.screenType() == .IPHONE_11_PRO ||  Base.tb.screenType() == .IPHONE_11_PRO_MAX {
            return true
        }
        return false
    }
    ...
    static func isIphoneXSScreen() -> Bool {
        if Base.tb.screenType() == .IPHONE_XS || Base.tb.screenType() == .IPHONE_XR || Base.tb.screenType() == .IPHONE_XS_Max {
            return true
        }
        return false
    }
...
public extension TBPOP where Base: UIDevice {
    ...
    static var isJailbroken: Bool {
        ...
        let path = String(format: "/private/%@", String.tb.stringWithUUID() ?? "")
        do {
