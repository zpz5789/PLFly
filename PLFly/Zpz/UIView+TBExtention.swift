...
extension UIView: TBPOPCompatible {}

// MARK: - 三、UIView 有关 Frame 的扩展
public extension TBPOP where Base: UIView {
    // MARK: 3.1、x 的位置
    /// x 的位置
    var x: CGFloat {
        get {
...
/**
 ...
 m44:初始为1
 */
extension TBPOP where Base: UIView {
...
public enum TBDashLineDirection: Int {
    case vertical = 0
    case horizontal = 1
}
...
public extension TBPOP where Base: UIView {
    ...
    func drawCircle(fillColor: UIColor, strokeColor: UIColor, strokeWidth: CGFloat) {
        let ciecleRadius = self.base.tb.width > self.base.tb.height ? self.base.tb.height : self.base.tb.width
    ...
    func drawDashLine(strokeColor: UIColor,
                       lineLength: Int = 4,
                      lineSpacing: Int = 4,
                        direction: TBDashLineDirection = .horizontal) {
        // 线粗
        let lineWidth = direction == .horizontal ? self.base.tb.height : self.base.tb.width
        ...
        if direction == .horizontal {
            ...
            // 终点
            // 横向 y = lineWidth / 2
            path.addLine(to: CGPoint(x: self.base.tb.width, y: lineWidth / 2))
        } else {
            path.move(to: CGPoint(x: lineWidth / 2, y: 0))
            // 终点
            // 纵向 Y = view 的height
            path.addLine(to: CGPoint(x: lineWidth / 2, y: self.base.tb.height))
        }
    ...
    func effectViewWithAlpha(alpha: CGFloat = 1.0, size: CGSize? = nil, style: UIBlurEffect.Style = .light) {
        ...
        let visualEffectView = UIVisualEffectView.tb.visualEffectView(size: visualEffectViewSize, alpha: alpha, style: style, isAddVibrancy: false)
...
// MARK: - 七、其他的方法
// 抖动方向枚举
public enum TBShakeDirection: Int {
...
public extension TBPOP where Base: UIView {
    ...
    func addWater(markText: String, textColor: UIColor = UIColor.black, font: UIFont = UIFont.systemFont(ofSize: 12)) {
        let waterMark: NSString = markText.tb.toNSString
        let textSize: CGSize = waterMark.size(withAttributes: [NSAttributedString.Key.font : font])
        // 多少行
        let line: NSInteger = NSInteger(self.base.tb.height * 3.5 / 80)
        // 多少列：自己的宽度/(每个水印的宽度+间隔)
        let row: NSInteger = NSInteger(self.base.tb.width / markText.tb.rectWidth(font: font, size: CGSize(width: self.base.tb.width, height: CGFloat(MAXFLOAT))))
        for i in 0..<line {
            for j in 0..<row {
    ...
    func shake(direction: TBShakeDirection = .horizontal, times: Int = 3, interval: TimeInterval = 0.1, delta: CGFloat = 2, completion: (() -> Void)? = nil) {
        // 播放动画
...
public extension TBPOP where Base: UIView {
    ...
    func getViewLayer(borderWidth: CGFloat = 0.5, borderColor: UIColor = .randomColor, backgroundColor: UIColor = .randomColor) {
        ...
        for subview in subviews {
            ...
            subview.tb.getViewLayer(borderWidth: borderWidth, borderColor: borderColor, backgroundColor: backgroundColor)
        }
        #endif
    }
    ...
    func findSubview(type: UIResponder.Type, resursion: Bool)-> UIView? {
        ...
        for e in self.base.subviews.enumerated() {
            let tmpView = e.element.tb.findSubview(type: type, resursion: resursion)
            if tmpView != nil {
                return tmpView
            }
        }
    ...
    func findSubView<T>(childViewType: T.Type) -> T? {
        ...
        for subview in self.base.subviews {
            if let view = subview.tb.findSubView(childViewType: childViewType) {
                return view
            }
        }
        return nil
    ...
    func findSubView(childViewClassName: String) -> UIView? {
        ...
        for subview in self.base.subviews {
            if let view = subview.tb.findSubView(childViewClassName: childViewClassName) {
                return view
            }
        }
        return nil
...
public extension TBPOP where Base: UIView {
    ...
    func addActionClosure(_ actionClosure: @escaping ViewClosure) {
        if let sender = self.base as? UIButton {
            sender.tb.setHandleClick(controlEvents: .touchUpInside) { (control) in
            ...
            sender.tb.addActionHandler({ (control) in
                actionClosure(nil, control, control.tag)
            }, for: .valueChanged)
        } else {
            _ = self.base.tb.addGestureTap { (reco) in
...
public extension TBPOP where Base : UIView {
    ...
    func gradientColor(_ direction: TBViewGradientDirection = .horizontal, _ gradientColors: [Any], _ gradientLocations: [NSNumber]? = nil, _ transform: CATransform3D? = nil) {
        // 获取渐变对象
        let gradientLayer = CAGradientLayer().tb.gradientLayer(direction, gradientColors, gradientLocations, transform)
        // 设置其CAGradientLayer对象的frame，并插入view的layer
        gradientLayer.frame = CGRect(x: 0, y: 0, width: self.base.tb.width, height: self.base.tb.height)
    ...
    ///   - gradientLocations: 决定每个渐变颜色的终止位置，这些值必须是递增的，数组的长度和 colors 的长度最好一致
    func gradientColorAnimation(direction: TBViewGradientDirection = .horizontal, startGradientColors: [Any], endGradientColors: [Any], duration: CFTimeInterval = 1.0, gradientLocations: [NSNumber]? = nil) {
        // 获取渐变对象
        let gradientLayer = CAGradientLayer().tb.gradientLayer(direction, startGradientColors, gradientLocations)
        // 设置其CAGradientLayer对象的frame，并插入view的layer
        gradientLayer.frame = CGRect(x: 0, y: 0, width: self.base.tb.width, height: self.base.tb.height)
        self.base.layer.insertSublayer(gradientLayer, at: 0)
